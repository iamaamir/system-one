    ...
    try {
      ...
      try {
        ...
        if (e?.name === "AbortError") {
          if (timedOut) throw new SystemOneTimeoutError(`request timed out after ${this.opts.timeoutMs}ms`);
          throw new SystemOneTransportError("request aborted", { cause: e });
        }
        throw new SystemOneTransportError(e?.message ?? "transport failure", { cause: e });
      }
      if (!res.ok) {
        throw new SystemOneHttpError(`provider error ${res.status}`, {
          status: res.status, provider: this.id, requestId: res.headers.get("x-request-id") ?? res.headers.get("x-typesafe-request-id") ?? undefined,
        });
      }
      ...
      out.requestId ??= res.headers.get("x-request-id") ?? res.headers.get("x-typesafe-request-id") ?? undefined;
