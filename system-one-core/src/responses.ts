...
export type AnswerFor<Q> = Q extends import("./questions.ts").NoulQuestion
  ? NoulAnswer
  : Q extends { type: "choice"; criteria: infer C }
    ? ChoiceAnswer<Extract<keyof C, string>>
    : Q extends import("./questions.ts").ScoreQuestion<any>
      ? ScoreAnswer
      : never;
export type AnswersFor<Q extends import("./questions.ts").QuestionMap> = {
  [K in keyof Q]: AnswerFor<Q[K]>;
};
export interface SystemOneResponse<
  Q extends
    import("./questions.ts").QuestionMap = import("./questions.ts").QuestionMap,
> {
  model?: string;
