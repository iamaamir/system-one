...
import { noul, choice } from "../src/questions.ts";
describe("mock provider", () => {
  it("returns canned answers through the client", async () => {
    const m = new MockSystemOneProvider({ answers: { q: { type: "noul", noul: 1 } } });
    const s1 = new SystemOne({ provider: m });
    const r = await s1.evaluate({ state: "x", questions: { q: noul("Is it?") } });
  ...
  it("throws on missing canned answer", async () => {
    ...
    await assert.rejects(() => new SystemOne({ provider: m }).evaluate({ state: "x", questions: { q: choice("W?", { a: null }) } }), /has no canned answer for/);
  });
  it("public exports resolve", async () => {
    const idx: any = await import("../src/index.ts");
    for (const k of ["SystemOne", "createSystemOne", "HttpSystemOneProvider", "MockSystemOneProvider", "choice", "noul", "score", "validateResponse"]) {
