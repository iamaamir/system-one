...
import { SystemOne, createSystemOne } from "../src/client.ts";
...
describe("client", () => {
  ...
  it("rejects empty questions", async () => {
    ...
    await assert.rejects(
      () => new SystemOne({ provider }).evaluate({ state: "x", questions: {} } as any),
      (e: unknown) => e instanceof SystemOneConfigurationError && (e as any).code === "SYSTEM_ONE_CONFIGURATION"
    );
  });
  it("createSystemOne factory delegates", async () => {
    const provider: SystemOneProvider = { id: "g", async evaluate() { return { answers: {}, metadata: { provider: "g" } } as any; } };
