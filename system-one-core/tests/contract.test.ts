...
const cases = [
  [
    "reflex",
    process.env.SYSTEM_ONE_TEST_REFLEX === "1",
    process.env.SYSTEM_ONE_REFLEX_URL ?? "http://localhost:8008",
    undefined,
    undefined,
  ],
  [
    "typesafe",
    process.env.SYSTEM_ONE_TEST_TYPESAFE === "1",
...
for (const [name, enabled, baseUrl, apiKey, model] of cases) {
  describe(`contract:${name}`, { skip: !enabled }, () => {
    it("answers mixed batch with valid protocol", async () => {
      const s1 = new SystemOne({
        provider: new HttpSystemOneProvider({ id: name, baseUrl, apiKey }),
      });
      ...
      if ((res.answers.team as any).type !== "choice")
        throw new Error("bad team");
      if ((res.answers.browser as any).type !== "noul")
        throw new Error("bad browser");
      if ((res.answers.level as any).type !== "score")
