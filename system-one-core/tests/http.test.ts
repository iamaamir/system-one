...
describe("http provider", () => {
  ...
  it("distinguishes user abort from timeout", async () => {
    const fakeFetch = async (_u: any, init: any) => new Promise((_res, rej) => {
      if (init.signal?.aborted) return rej(Object.assign(new Error("aborted"), { name: "AbortError" }));
      init.signal?.addEventListener("abort", () => rej(Object.assign(new Error("aborted"), { name: "AbortError" })));
    });
    const p = new HttpSystemOneProvider({ baseUrl: "https://x.example", timeoutMs: 5000, fetch: fakeFetch as any });
    const ac = new AbortController();
    ac.abort();
    try {
      await p.evaluate({ state: "x", questions: { c: choice("W?", { a: null }) } }, { signal: ac.signal });
      assert.fail("should throw");
    } catch (e: any) {
